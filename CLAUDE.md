...
## 현재 상태 (이어받을 때 먼저 확인)
- 총 상품 **1007개** (`src/products.json`), 다음 `id`는 **1028**부터. (실제 값은 `python3 -c "import json;d=json.load(open('src/products.json'));print(len(d),max(x['id'] for x in d))"`로 확인)
...
## 문구 톤 (규칙 — 반드시 지킬 것)
- **앱의 모든 사용자 노출 문구는 따뜻하고 친근한 톤**으로 쓴다. (안내·빈결과·준비중·에러·버튼 등 전부)
- 사무적·딱딱한 표현 금지. 예) "검색 결과가 없습니다" ❌ → "아직 찾으신 조건에 맞는 제품이 없어요. 더 많은 제품을 얼른 등록할게요!" ✅
- 0건/미등록 상황은 사과 + 개선 약속(부지런히 등록할게요) 톤. 이모지(🌱 등) 가볍게 활용 OK.
- 새 문구를 넣거나 기존 문구를 고칠 때 이 톤을 항상 유지한다.
